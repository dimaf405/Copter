...
#define debug_can(level_debug, fmt, args...)                        \
    do                                                              \
    {                                                               \
        AP::can().log_text(level_debug, "ToshibaCAN", fmt, ##args); \
    } while (0)
#else
#define debug_can(level_debug, fmt, args...)
#endif
...
const uint16_t Fire_CAN::TOSHIBACAN_SEND_TIMEOUT_US = 500;
...
Fire_CAN *Fire_CAN::get_tcan(uint8_t driver_index)
{
    ...
    return static_cast<Fire_CAN *>(AP::can().get_driver(driver_index));
}

bool Fire_CAN::add_interface(AP_HAL::CANIface *can_iface)
{
...
void Fire_CAN::init(uint8_t driver_index, bool enable_filters)
{
    ...
    if (!hal.scheduler->thread_create(FUNCTOR_BIND_MEMBER(&Fire_CAN::loop, void), _thread_name, 4096, AP_HAL::Scheduler::PRIORITY_MAIN, 1))
    {
            ...
            while (read_frame(recv_frame, timeout))
            {
                ...
                if ((recv_frame.id >= MOTOR_DATA1) && (recv_frame.id <= MOTOR_DATA1 + TOSHIBACAN_MAX_NUM_ESCS))
                {
                    ...
                    if (esc_id < TOSHIBACAN_MAX_NUM_ESCS)
                    {
                        ...
                        update_telem_data(esc_id, t,
                                          AP_ESC_Telem_Backend::TelemetryType::CURRENT | AP_ESC_Telem_Backend::TelemetryType::VOLTAGE | AP_ESC_Telem_Backend::TelemetryType::CONSUMPTION);
                ...
                if ((recv_frame.id >= MOTOR_DATA2) && (recv_frame.id <= MOTOR_DATA2 + TOSHIBACAN_MAX_NUM_ESCS))
                {
                    ...
                    if (esc_id < TOSHIBACAN_MAX_NUM_ESCS)
                    {
                        ...
                        update_telem_data(esc_id, t, AP_ESC_Telem_Backend::TelemetryType::MOTOR_TEMPERATURE | AP_ESC_Telem_Backend::TelemetryType::TEMPERATURE);
                    }
                }
...
bool Fire_CAN::write_frame(AP_HAL::CANFrame &out_frame, uint64_t timeout)
{
...
bool Fire_CAN::read_frame(AP_HAL::CANFrame &recv_frame, uint64_t timeout)
{
...
void Fire_CAN::update_esc_present_bitmask()
{
...
Fire_CAN::motor_request_data_cmd_t Fire_CAN::get_motor_request_data_cmd(uint8_t request_id) const
{
