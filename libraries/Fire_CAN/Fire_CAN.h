...
class Fire_CAN : public AP_CANDriver, public AP_ESC_Telem_Backend
{
...
public:
    ...
    /* Do not allow copies */
    Fire_CAN(const Fire_CAN &other) = delete;
    Fire_CAN &operator=(const Fire_CAN &) = delete;

    // Return ToshibaCAN from @driver_index or nullptr if it's not ready or doesn't exist
    static Fire_CAN *get_tcan(uint8_t driver_index);
